...
public class GreedySearch 
{
		...
		public Node search(boolean check_state)
		{
			...
			SortedFrontier frontier = new SortedFrontier(SortBy.h);       //children nodes are stored here
			...
			{			
				Node node1 = frontier.removeTop();			            // gets the next node on the frontier
					...
					else 
					{
						...
						node.expand(h);               //not the goal node so we expand the node so that the children node are added to frontier
					...
					if (check_state == true)               //checks if true then it adds explored sets to the sorted frontier and an entry is created in explored
					{
						...
						{
							if(explored.contains(x.loc) || frontier.contains(x))
							junk.add(x);
						}
								
							frontier.addSorted(node.children);      //if the child is in the explored set, the child is not added to the frontier
						
					}
					
					else
					{
						frontier.addSorted(node.children);               //adds the children node to the frontier
					}
